...
type InformersMap struct {
	...
	// namespace is the namespace that the informers listwatchers are restricted to
	// default means all namespaces or cluster scoped
	namespace string
}
...
func (ip *InformersMap) newListWatch(gvk schema.GroupVersionKind) (*cache.ListWatch, error) {
	...
	return &cache.ListWatch{
		ListFunc: func(opts metav1.ListOptions) (runtime.Object, error) {
			...
			err := client.Get().Namespace(ip.namespace).Resource(mapping.Resource).VersionedParams(&opts, ip.paramCodec).Do().Into(res)
			return res, err
		},
		...
		WatchFunc: func(opts metav1.ListOptions) (watch.Interface, error) {
			...
			opts.Watch = true
			return client.Get().Namespace(ip.namespace).Resource(mapping.Resource).VersionedParams(&opts, ip.paramCodec).Watch()
		},
	}, nil
}
